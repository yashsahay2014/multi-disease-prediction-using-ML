...
for s in total_symptoms:
    ...
    for comb in range(1, len(symp)+1):
        for subset in combinations(symp, comb):
            subset=' '.join(subset)
            # synonyms() function defined above is used here
            subset = synonymsFinder(subset)
...
# If Jaccard>threshold, it means that those synonyms are similar and one of them can be 
